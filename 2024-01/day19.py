...
def parse(raw):
    raw = raw.strip()
    towel_part, patterns_part = raw.split("\n\n")
    towels = [t.strip() for t in towel_part.split(",")]
    patterns = patterns_part.split("\n")
    return towels, patterns


def solve(towels: list[str], pattern: str) -> bool:
    queue = {pattern}
    seen: set[str] = set()

    while queue:
        pattern: str = min(queue, key=len)
        queue.remove(pattern)
        print(len(queue), pattern, sep="\t")
        if pattern in seen:
            continue
        seen.add(pattern)
